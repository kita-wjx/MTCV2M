...
def range_to_mask(mask: torch.Tensor, ranges: torch.Tensor, mode: str = 'mask0') -> torch.Tensor:
    assert len(mask.shape) == 2, "Mask shape should be 2 dimensional."
    assert len(ranges.shape) == 2, "Ranges shape should be 2 dimensional."
    assert mask.shape[0] == ranges.shape[0], "Mask and Ranges should have the same batch size."

    batch_size, _ = ranges.shape
    col_indices = torch.arange(mask.shape[1]).unsqueeze(0).repeat(batch_size, 1).to(ranges.device)

    if mode == 'mask0':
        condition = (col_indices >= ranges[:, 0].unsqueeze(1)) & (col_indices <= ranges[:, 1].unsqueeze(1))
    elif mode =='mask1':
        condition = (col_indices < ranges[:, 0].unsqueeze(1)) | (col_indices > ranges[:, 1].unsqueeze(1))
    else:
    ...
    ones = torch.where(condition, torch.zeros_like(ones), ones)
    mask *= ones
...
if __name__ == '__main__':
    ...
    ranges = torch.tensor([[0, 3], [2, 4], [0, 6], [5, 8], [8, 9]])
    print(range_to_mask(mask, ranges, mode="mask1"))
