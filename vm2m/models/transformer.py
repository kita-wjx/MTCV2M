...
class StreamingMultiheadAttention(StreamingModule):
    ...
    def __init__(self, embed_dim: int, num_heads: int, dropout: float = 0.0, bias: bool = True,
                 ...
                 device=None, dtype=None, segment_cross_attention: bool = False, block_size: tp.Optional[int] = None):
        super().__init__()
        ...
        self.segment_cross_attention = segment_cross_attention
        ...
        if segment_cross_attention:
            assert block_size is not None, "Block size must be specified for segment cross attention."
    ...
    def _get_segment_mask(self, blcoks_size: int, device: torch.device, dtype: torch.dtype, q_len: int, k_len: int, offsets: torch.Tensor = None):
        blocks_num = k_len // blcoks_size
        if q_len != 1:
            frame_rate = q_len // k_len
        mask = torch.zeros(q_len, k_len, dtype=bool, device=device)
        if q_len != 1:
            for i in range(blocks_num):
                if i == blocks_num - 1:
                    mask[i*blcoks_size*frame_rate:, i * blcoks_size:] = True
                else:
                    mask[i*blcoks_size*frame_rate: (i+1)*blcoks_size*frame_rate, i*blcoks_size: (i+1)*blcoks_size] = True
        else:
            block_th = (offsets[0] // 50 // blcoks_size).item()
            if (block_th+1)*blcoks_size < k_len:
                mask[0, block_th * blcoks_size:(block_th+1)*blcoks_size] = True
            else:
                mask[0, (block_th-1) * blcoks_size:] = True
        return torch.where(
            mask,
            torch.zeros([], device=device, dtype=dtype),
            torch.full([], float('-inf'), device=device, dtype=dtype))
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor,
                ...
                average_attn_weights=True, is_causal=False, is_segment_attention=False, wav_length=None, offsets=None):
        ...
        if self.custom:
            ...
            if self.memory_efficient:
                ...
                if _efficient_attention_backend == 'torch':
                    if is_segment_attention and self.segment_cross_attention:
                        assert wav_length is not None
                        q_cut = q[:,:,:-wav_length,:]
                        q = q[:,:,-wav_length:,:]
                        cross_atten_mask = self._get_segment_mask(self.block_size, q.device, q.dtype, q.shape[2], k.shape[2], offsets)
                        # with open("./mask.txt", "w") as f:
                        #     for i in range(cross_atten_mask.shape[0]):
                        #         for j in range(cross_atten_mask.shape[1]):
                        #             f.write(str(cross_atten_mask[i,j].item()) + " ")
                        #         f.write("\n")
                        x = torch.nn.functional.scaled_dot_product_attention(
                            q, k, v, attn_mask=cross_atten_mask, is_causal=False, dropout_p=p)
                        x = torch.cat([q_cut, x], dim=2)
                    else:
                        x = torch.nn.functional.scaled_dot_product_attention(
...
class StreamingTransformerLayer(nn.TransformerEncoderLayer):
    ...
    """
    def __init__(self, idx: int, d_model: int, num_heads: int, dim_feedforward: int = 2048, dropout: float = 0.1,
                 ...
                 kv_repeat: int = 1, norm: str = 'layer_norm', device=None, dtype=None, segment_cross_attention: bool = False, **kwargs):
        block_sizes = kwargs.pop('blocks_size', None)
        ...
        if cross_attention or self.segment_cross_attention:
            if cross_attention:
                self.cross_attention = StreamingMultiheadAttention(
                    cross_attention=True, qk_layer_norm=qk_layer_norm_cross,
                    segment_cross_attention=segment_cross_attention, block_size=None,
                    **attn_kwargs, **factory_kwargs)
            elif self.segment_cross_attention:
                block_size = block_sizes[self.idx % len(block_sizes)]
                if block_size == 0:
                    block_size = None
                self.cross_attention = StreamingMultiheadAttention(
                    cross_attention=True, qk_layer_norm=qk_layer_norm_cross,
                    segment_cross_attention=segment_cross_attention if block_size is not None else False, block_size=block_size,
                    **attn_kwargs, **factory_kwargs)
            # Norm and dropout
    ...
    def _cross_attention_block(self, src: torch.Tensor,
                               cross_attention_src: torch.Tensor, wav_length: tp.Optional[int], offsets: torch.Tensor = None) -> torch.Tensor:
        assert self.cross_attention is not None
        # queries are from src, keys and values from cross_attention_src.
        if self.segment_cross_attention:
            x = self.cross_attention(
                src, cross_attention_src, cross_attention_src, need_weights=False, is_segment_attention=True, wav_length=wav_length, offsets=offsets)[0]
        else:
            x = self.cross_attention(
    ...
    def forward(self, src: torch.Tensor, src_mask: tp.Optional[torch.Tensor] = None,  # type: ignore
                ...
                cross_attention_src: tp.Optional[torch.Tensor] = None, wav_length: tp.Optional[int] = None, offsets: torch.Tensor = None):
        ...
        if self.norm_first:
            ...
            if cross_attention_src is not None and self.cross_attention is not None:
                x = x + self.layer_scale_cross(
                    self._cross_attention_block(
                        self.norm_cross(x), cross_attention_src, wav_length=wav_length, offsets=offsets))
            ...
            if cross_attention_src is not None and self.cross_attention is not None:
                x = self.norm_cross(
                    x + self.layer_scale_cross(
                        self._cross_attention_block(src, cross_attention_src, wav_length=wav_length, offsets=offsets)))
            x = self.norm2(x + self.layer_scale_2(self._ff_block(x)))
        return x

class StreamingCrossAttenLayer(nn.TransformerEncoderLayer):
    """Args:
        d_model (int): Dimension of the data.
        num_heads (int): Number of heads.
        dim_feedforward (int): Intermediate dimension of FF module.
    ...
    def __init__(self, idx: int, d_model: int, num_heads: int, dim_feedforward: int = 2048, dropout: float = 0.1,
        ...
        # zero linear
        self.zero_linear1 = nn.Linear(d_model, d_model, bias=bias_ff, **factory_kwargs)
        self.zero_linear1.weight.data.zero_()
        if bias_ff:
            self.zero_linear1.bias.data.zero_()
        self.zero_linear2 = nn.Linear(d_model, d_model, bias=bias_ff, **factory_kwargs)
        self.zero_linear2.weight.data.zero_()
        if bias_ff:
            self.zero_linear2.bias.data.zero_()
    ...
    def forward(self, src: torch.Tensor, in_atten_src: tp.Optional[torch.Tensor] = None, cross_attention_src: tp.Optional[torch.Tensor] = None, 
                src_key_padding_mask: tp.Optional[torch.Tensor] = None, src_mask: tp.Optional[torch.Tensor] = None, wav_length: tp.Optional[int] = None, offsets: torch.Tensor = None):
        if self.cross_attention is None:
            if self.segment_cross_attention:
                assert cross_attention_src is not None
            else:
            ...
            assert cross_attention_src is not None
        x = self.zero_linear1(in_atten_src)
        x += src
        ...
        if self.norm_first:
            ...
            if cross_attention_src is not None:
                x = self.norm_cross(
                    x + self.layer_scale_cross(
                        self._cross_attention_block(x, cross_attention_src, wav_length=wav_length, offsets=offsets)))
            x = self.norm2(x + self.layer_scale_2(self._ff_block(x)))
        x = self.zero_linear2(x)
        return x
...
class StreamingTransformer(StreamingModule):
    ...
    def __init__(self, d_model: int, num_heads: int, num_layers: int, dim_feedforward: int = 2048,
                 ...
                 checkpointing: str = 'none', device=None, dtype=None, segment_cross_attention: bool = False, muticond: bool = False, **kwargs):
        super().__init__()
        ...
        for idx in range(num_layers):
            self.layers.append(
                layer_class(
                    idx=idx, d_model=d_model, num_heads=num_heads, dim_feedforward=dim_feedforward,
                    ...
                    device=device, dtype=dtype, segment_cross_attention=segment_cross_attention, **kwargs))
        
        self.muticond_layers = nn.ModuleList()
        if muticond:
            for idx in range(num_layers // 4):
                self.muticond_layers.append(
                    StreamingCrossAttenLayer(
                        idx=idx, d_model=d_model, num_heads=num_heads, dim_feedforward=dim_feedforward,
                        dropout=dropout, bias_ff=bias_ff, bias_attn=bias_attn, 
                        causal=causal, past_context=past_context, custom=custom,
                        memory_efficient=memory_efficient, attention_as_float32=attention_as_float32,
                        cross_attention=True, layer_scale=layer_scale, rope=self.rope,
                        device=device, dtype=dtype, segment_cross_attention=False, **kwargs))
    ...
    def forward(self, x: torch.Tensor, in_atten_src: torch.Tensor, *args, **kwargs):
        B, T, C = x.shape
        if in_atten_src is not None:
            _, in_atten_t, _ = in_atten_src.shape
        ...
        for idx, layer in enumerate(self.layers):
            if in_atten_src is not None and idx % 4 == 0:
                x_cond_src = x[:, -in_atten_t:, :].clone()
                x_cond_src = self.muticond_layers[idx // 4](x_cond_src, in_atten_src, *args, **kwargs)
                
            x = self._apply_layer(layer, x, *args, **kwargs)
            
            if in_atten_src is not None and idx % 4 == 0:
                x[:, -in_atten_t:, :] += x_cond_src
