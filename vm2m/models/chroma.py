...
class ChromaExtractor(nn.Module):
    ...
    def __init__(self, sample_rate: int, n_chroma: int = 12, radix2_exp: int = 12, nfft: tp.Optional[int] = None,
        ...
        super().__init__()
        self.winlen = winlen or 2 ** radix2_exp # 2^14 = 16384
        self.nfft = nfft or self.winlen # 2^14 = 16384
    ...
    def forward(self, wav: torch.Tensor) -> torch.Tensor:
        T = wav.shape[-1] # B, 1, sec * 32000(960000)
        ...
        spec = self.spec(wav).squeeze(1) # B, 8193(related to nfft), 235(960000//winhop+1)
        raw_chroma = torch.einsum('cf,...ft->...ct', self.fbanks, spec) # B, 12, 235
