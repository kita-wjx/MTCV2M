...
def load_lm_model(file_or_url_or_id: tp.Union[Path, str], device='cpu', cache_dir: tp.Optional[str] = None):
    ...
    cfg = OmegaConf.create(pkg['xp.cfg'])
    if 'self_wav' in cfg.conditioners.keys() and 'style' in cfg.conditioners.self_wav.keys():
        cfg.conditioners.self_wav.style.encodec_checkpoint = os.path.join(file_or_url_or_id, 'compression_state_dict.bin')
    cfg.device = str(device)
    if cfg.device == 'cpu':
        cfg.dtype = 'float32'
    else:
