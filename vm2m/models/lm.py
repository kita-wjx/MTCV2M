...
class LMModel(StreamingModule):
    ...
    def __init__(self, pattern_provider: CodebooksPatternProvider, condition_provider: ConditioningProvider,
                 ...
                 muticondfuser: MutiConditionFuser = None,
                 **kwargs):
        super().__init__()
        ...
        self.fuser = fuser
        self.muticondfuser = muticondfuser
        self.card = card
    ...
    def _init_weights(self, weight_init: tp.Optional[str], depthwise_init: tp.Optional[str], zero_bias_init: bool):
        ...
        for layer_idx, tr_layer in enumerate(self.transformer.muticond_layers):
            depth = None
            if depthwise_init == 'current':
                depth = layer_idx + 1
            elif depthwise_init == 'global':
                depth = len(self.transformer.muticond_layers)
            init_fn = partial(init_layer, method=weight_init, init_depth=depth, zero_bias_init=zero_bias_init)
            tr_layer.apply(init_fn)
    ...
    def forward(self, sequence: torch.Tensor,
        ...
        if condition_tensors is None:
            ...
            if self.muticondfuser:
                condition_tensors = self.muticondfuser(condition_tensors)
        else:
            assert not conditions, "Shouldn't pass both conditions and condition_tensors."
        # print("input_: ", input_.shape)
        # print("condition_tensors1: ", condition_tensors["visual_content"][0].shape)
        # print("condition_tensors2: ", condition_tensors["finegrained_content"][0].shape)
        # print("condition_tensors3: ", condition_tensors["self_wav"][0].shape)
        input_, cross_attention_input, in_atten = self.fuser(input_, condition_tensors)

        out = self.transformer(input_, cross_attention_src=cross_attention_input, in_atten_src=in_atten,
                               src_mask=(self.attn_mask_per_stage[stage] if stage >= 0 else None), wav_length = S,
                               offsets=self.fuser._streaming_state['offsets']-1 if S==1 else None)  # type: ignore
        # out:[B, S+prepend, dim]
        if self.out_norm:
            out = self.out_norm(out)
        logits = torch.stack([self.linears[k](out) for k in range(K)], dim=1)  # [B, K, S+prepend, card] or [B, K, S, card]
    ...
    def _sample_next_token(self,
        ...
        Args:
            ...
            cfg_coef_beta (float, optional): If None, simple classifier free guidance is used with cfg_coef.
                ...
                push the video condition more than the style condition in the case where both video and style
        ...
        if cfg_coef_beta is not None:
            ...
            if condition_tensors:
                # Preparing for CFG, predicting conditional video and style, conditional style
                # and unconditional
    ...
    def generate(self,
        ...
        if cfg_coef_beta is not None:
            if conditions:
                wav_conditions = _drop_video_condition(conditions)
                null_conditions = ClassifierFreeGuidanceDropout(p=1.0)(conditions)
                conditions = conditions + wav_conditions + null_conditions
                tokenized = self.condition_provider.tokenize(conditions)
                cfg_conditions = self.condition_provider(tokenized)
                if self.muticondfuser is not None:
                    cfg_conditions = self.muticondfuser(cfg_conditions)
        elif conditions:
            two_step_cfg = self.two_step_cfg if two_step_cfg is None else two_step_cfg
            if conditions:
                null_conditions = ClassifierFreeGuidanceDropout(p=1.0)(conditions)
