...
from ..utils.environment import AudioCraftEnvironment
...
class CLAPTextConsistencyMetric(TextConsistencyMetric):
    ...
    def update(self, audio: torch.Tensor, text: tp.List[str], sizes: torch.Tensor, sample_rates: torch.Tensor, save_every=True) -> None:
        """Compute cosine similarity between audio and text pairs and accumulate scores over the dataset."""
        assert audio.size(0) == text.size(0), "Number of audio and text samples should match"
        ...
        text = convert_audio(text, from_rate=sample_rate, to_rate=self.model_sample_rate, to_channels=1).mean(dim=1)
        audio_embeddings = self.model.get_audio_embedding_from_data(audio, use_tensor=True)
        text_embeddings = self.model.get_audio_embedding_from_data(text, use_tensor=True)
        # cosine similarity between the text and the audio embedding
        if save_every:
            cs_data = []
        for i in range(audio.size(0)):
            cosine_sim = torch.nn.functional.cosine_similarity(audio_embeddings[i:i+1,:], text_embeddings[i:i+1,:], dim=1, eps=1e-8)
            self.cosine_sum += cosine_sim.sum(dim=0)  # type: ignore
            self.weight += torch.tensor(cosine_sim.size(0))  # type: ignore
            if save_every:
                cs_data.append(str(cosine_sim.sum(dim=0).item()))
        if save_every:
            self.cs_list = cs_data
    ...
    def compute(self, save_every=False):
        ...
        if save_every:
            return (self.cosine_sum / self.weight).item(), self.cs_list
        return (self.cosine_sum / self.weight).item()  # type: ignore
    
    def extract(self, audio: torch.Tensor, text: tp.List[str], sample_rates: torch.Tensor) -> None:
        """Compute cosine similarity between audio and text pairs and accumulate scores over the dataset."""
        assert audio.size(0) == text.size(0), "Number of audio and text samples should match"
        assert torch.all(sample_rates == sample_rates[0].item()), "All items in batch should have the same sample rate"
        sample_rate = int(sample_rates[0].item())
