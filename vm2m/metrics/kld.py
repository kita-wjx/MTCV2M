...
class KLDivergenceMetric(torchmetrics.Metric):
    ...
    def __init__(self):
        ...
        self.add_state("kld_list", default=[])
    ...
    def update(self, preds: torch.Tensor, targets: torch.Tensor,
        ...
        if preds_probs is not None and targets_probs is not None:
            ...
            if save_every == True:
                self.kld_list = [str(item) for item in kld_scores.tolist()]

    def compute(self, save_every = False) -> dict:
        ...
        if save_every:
            return {'kld': kld_pq, 'kld_pq': kld_pq, 'kld_qp': kld_qp, 'kld_both': kld_both, 'kld_list': self.kld_list}
