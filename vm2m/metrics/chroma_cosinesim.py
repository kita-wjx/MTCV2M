...
from ..models.chroma import ChromaExtractor
...
class ChromaCosineSimilarityMetric(torchmetrics.Metric):
    ...
    def __init__(self, sample_rate: int, n_chroma: int, radix2_exp: int, argmax: bool, eps: float = 1e-8):
        ...
        self.add_state("cs_list", default=[])

    def update(self, preds: torch.Tensor, targets: torch.Tensor,
               sizes: torch.Tensor, sample_rates: torch.Tensor, save_every = False) -> None:
        ...
        for i in range(len(gt_chroma)):
            ...
            if save_every:
                cs_data.append(str((cosine_sim.sum(dim=0) / torch.tensor(t)).item()))
        if save_every:
            self.cs_list = cs_data
    def compute(self, save_every = False) -> float:
