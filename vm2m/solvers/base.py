...
class StandardSolver(ABC, flashy.BaseSolver):
    ...
    def cpoy_weight_to_muticond(self):
        layers_num = len(self.model.transformer.layers)
        muti_leyers_num = len(self.model.transformer.muticond_layers)
        assert layers_num == muti_leyers_num * 4
        for i in range(0, layers_num, 4):
            ori_layer = self.model.transformer.layers[i]
            target_layer = self.model.transformer.muticond_layers[i // 4]

            target_layer.self_attn.out_proj.load_state_dict(ori_layer.self_attn.out_proj.state_dict())
            target_layer.linear1.load_state_dict(ori_layer.linear1.state_dict())
            target_layer.linear2.load_state_dict(ori_layer.linear2.state_dict())
            target_layer.cross_attention.out_proj.load_state_dict(ori_layer.cross_attention.out_proj.state_dict())
            target_layer.norm1.load_state_dict(ori_layer.norm1.state_dict())
            target_layer.norm2.load_state_dict(ori_layer.norm2.state_dict())
            target_layer.norm_cross.load_state_dict(ori_layer.norm_cross.state_dict())
