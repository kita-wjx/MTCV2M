...
from ..models.conditioners import SegmentWithAttributes, WavCondition, \
            StyleConditioner, _drop_video_condition
...
class MusicGenSolver(base.StandardSolver):
    ...
    def __init__(self, cfg: omegaconf.DictConfig):
        ...
        self.fine_tune = self.cfg.optim.fine_tune
    ...
    def build_model(self) -> None:
        ...
        if self.cfg.optim.fine_tune:
        # freeze some weight
            for name, param in self.model.named_parameters():
                param.requires_grad = False
            
            # layer_idxs = [idx for idx in range(0, 48, 4)] # jump freeze
            for name, param in self.model.named_parameters():
                # for idx in layer_idxs:
                #     if name.startswith(f"transformer.layers.{idx}."):
                #         param.requires_grad = True
                # if name.startswith("out_norm") or name.startswith("linears"):
                #     param.requires_grad = True
                if name.startswith("condition_provider.conditioners.intensity") or name.startswith("condition_provider.conditioners.beat") or name.startswith("condition_provider.conditioners.melody") or name.startswith("condition_provider.conditioners.va"):
                    param.requires_grad = True
                if name.startswith("transformer.muticond_layers"):
                    param.requires_grad = True
                if name.startswith("muticondfuser.dwnet.blocks"):
                    param.requires_grad = True
    ...
    def _prepare_tokens_and_attributes(
        ...
        with self.autocast:
            ...
            if self.model.muticondfuser is not None:
                condition_tensors = self.model.muticondfuser(condition_tensors)
        # print(condition_tensors['visual_content'][0].shape)
        # print(condition_tensors['visual_content'][1].shape)
        # print(condition_tensors['self_wav'][0].shape)
        # print(condition_tensors['self_wav'][1].shape)
    ...
    def run_generate_step(self, batch: tp.Tuple[torch.Tensor, tp.List[SegmentWithAttributes]],
        ...
        if remove_text_conditioning:
            attributes = _drop_video_condition(attributes)
